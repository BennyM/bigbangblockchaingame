...
public class BlockchainChangeProcessor
{
    ...
    public async Task Process()
    {
        ...
        var test = await web3.Eth.GetBalance.SendRequestAsync(_account.Value.MasterAccountAddress);
        var moreTest = test.Value;
        ...
        try
        {
            var receipts = await transactionPolling.SendRequestsAsync(transactionInputs);
