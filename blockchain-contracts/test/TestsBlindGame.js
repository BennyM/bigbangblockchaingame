...
contract('BlindGame', function(accounts) {
  ...
  it("draw and resolve", function(done) {
    var account0 = accounts[0];
    var account1 = accounts[1];
    ...
    var game;
    BlindGame.new(account0, account1, hash0, hash0, {from: account0})
      .then(function(createdGame){
        game = createdGame;
        assert.isOk(game);
        console.log("asserted game")
      ...
      .then(function(tx){
        ...
        assert.isOk(tx.receipt);
        console.log('asserted revealed hand');
        return game.revealHand(1, 'salt1', {from: account1});
      })
      .then(function(tx){
        assert.isOk(tx.receipt);
        return game.winner.call();
      ...
      .then(function(winner){
        assert.equal('0x0000000000000000000000000000000000000000', winner);
        return game.playHand(hash0, {from : account0});
      })
      .then(function(tx){
         return game.playHand(hash1, {from : account1});
      })
      .then(function(tx){
        return game.revealHand(1, 'salt1', {from: account0});
      })
...
it(" 2 draw and resolve", function(done) {
    var account0 = accounts[0];
    var account1 = accounts[1];
    ...
    BlindGame.new(account0, account1, hash0, hash0, {from: account0})
      ...
      .then(function(tx){
         return game.playHand(hash0, {from : account1});
      })
      .then(function(tx){
        return game.revealHand(1, 'salt1', {from: account0});
      })
